...
const products = [
  ...
  { brand: "Datecs", type: "Фискални принтери", name: "Datecs FP-50 X", category: "Компактен серия X", description: "Компактен фискален принтер от серията X — наследник на FP-650 с подобрена свързаност и по-малък форм-фактор." },
  ...
  { brand: "Datecs", type: "Термопринтери", name: "Datecs DPP-250", category: "Мобилен термопринтер", description: "Компактен мобилен термопринтер за куриерски услуги, логистика и мобилна дейност на терен." },
  ...
  { brand: "Datecs", type: "Термопринтери", name: "Datecs EP-50 MX", category: "Етикетиращ принтер", description: "Мобилен етикетиращ принтер с компактен дизайн за печат на баркод и ценови етикети на терен." },
  ...
  { brand: "Tremol", type: "Касови апарати", name: "Tremol S25", category: "Касов апарат", description: "Компактен и надежден касов апарат Tremol S25. Малък, лек и лесен за употреба — с Bluetooth, GPRS и батерия за работа на терен." },
  { brand: "Tremol", type: "Касови апарати", name: "Tremol S21", category: "Касов апарат", description: "Tremol S21 — функционален касов апарат с GPRS и Bluetooth за стационарна и мобилна употреба. Подходящ за разнообразни типове търговски обекти." },
  { brand: "Tremol", type: "Касови апарати", name: "Tremol A19 Plus", category: "Касов апарат", description: "Tremol A19 Plus — касов апарат с вграден GPRS модем и Bluetooth за разнообразни търговски обекти. За магазини, заведения и сервизни обекти." },
  { brand: "Tremol", type: "Касови апарати", name: "Tremol M20", category: "Касов апарат", description: "Tremol M20 — надежден касов апарат с GPRS и Bluetooth за малки и средни търговски обекти. Балансиран избор за средни по натоварване обекти." },
  { brand: "Tremol", type: "Касови апарати", name: "Tremol M23", category: "Касов апарат", description: "Tremol M23 — висок клас касов апарат с GPRS, Bluetooth и LAN — за натоварени обекти с детайлна отчетност. Подходящ за по-натоварени търговски обекти с нужда от детайлна отчетност." },
  // ─── TREMOL — ФИСКАЛНИ ПРИНТЕРИ ─────────────────────────────────────────────
  { brand: "Tremol", type: "Фискални принтери", name: "Tremol FP24", category: "Фискален принтер", description: "Tremol FP24 — достъпен фискален принтер с RS-232 и USB — лесна интеграция с всеки търговски софтуер. Подходящ за обекти с умерено натоварване." },
  { brand: "Tremol", type: "Фискални принтери", name: "Tremol FP01", category: "Фискален принтер", description: "Tremol FP01 — фискален принтер с висока скорост, RS-232, USB и LAN за магазини и търговски обекти. За магазини и търговски обекти." },
  { brand: "Tremol", type: "Фискални принтери", name: "Tremol FP15", category: "Висок клас фискален принтер", description: "Tremol FP15 — висок клас фискален принтер с WiFi, LAN и USB за натоварени обекти с висока производителност. За натоварени обекти с изисквания за висока производителност." },
  { brand: "Tremol", type: "Фискални принтери", name: "Tremol FP21", category: "Фискален принтер", description: "Tremol FP21 — надежден фискален принтер с RS-232, USB и LAN за лесна интеграция с търговски софтуер." },
];
...
const faqItems = [
  ...
  { question: "С кои производители работите?", answer: "Изодимпекс ЕООД е официален дистрибутор на всички четири водещи производителя на фискални устройства в България.", brands: true },
...
const STYLES = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .filter-label { font-size: 10px; font-weight: 700; letter-spacing: 0.2em; text-transform: uppercase; color: var(--muted); flex-shrink: 0; width: 72px; }
  .filter-divider { width: 1px; height: 20px; background: var(--border); flex-shrink: 0; }
  .filter-pills { display: flex; flex-wrap: wrap; gap: 8px; }
  .fpill { font-size: 12px; font-weight: 600; padding: 7px 18px; border: 1px solid var(--border); background: transparent; color: var(--muted2); cursor: pointer; letter-spacing: 0.06em; transition: all 0.18s; font-family: inherit; clip-path: polygon(5px 0%, 100% 0%, calc(100% - 5px) 100%, 0% 100%); }
  .fpill:hover:not(.active) { border-color: var(--border-amber); color: var(--amber); background: var(--amber-glow); }
  .fpill.active { background: var(--amber); color: #000; border-color: var(--amber); font-weight: 800; }
  ...
  .finput { background: var(--bg3); border: 1px solid var(--border); padding: 13px 16px; font-size: 14px; color: var(--text); outline: none; transition: border-color 0.2s; font-family: inherit; width: 100%; appearance: none; }
  ...
  @media (max-width: 768px) {
    ...
    .filter-label { width: 100% !important; margin-bottom: 4px; }
    .fpill { font-size: 11px !important; padding: 6px 12px !important; clip-path: polygon(4px 0%, 100% 0%, calc(100% - 4px) 100%, 0% 100%) !important; }
...
function ContactForm() {
  const [form, setForm] = useState({ type: "", name: "", company: "", phone: "", email: "", message: "" });
  ...
  const validate = () => {
    ...
    else if (!/^[\d\s\+\-\(\)]{6,}$/.test(form.phone.trim())) e.phone = "Невалиден телефонен номер";
    if (!form.email.trim()) e.email = "Имейлът е задължителен";
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())) e.email = "Невалиден имейл адрес";
    return e;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xbdpybnv", {
        method: "POST",
        headers: { "Content-Type": "application/json", "Accept": "application/json" },
        body: JSON.stringify({
          "Вид запитване": form.type || "Не е посочено",
          "Име": form.name || "Не е посочено",
          "Фирма": form.company || "Не е посочено",
          "Телефон": form.phone,
          "Имейл": form.email,
          "Съобщение": form.message || "Без съобщение",
        })
      });
      if (res.ok) {
        setSubmitted(true);
      } else {
        alert("Грешка при изпращане. Моля опитайте отново или се свържете с нас на телефон.");
      }
    } catch (err) {
      alert("Грешка при изпращане. Проверете интернет връзката си и опитайте отново.");
    }
  };
  ...
  if (submitted) return (
    <div style={{ padding: "48px 32px", border: "1px solid var(--border-amber)", background: "var(--amber-glow)", textAlign: "center" }}>
      ...
      <button onClick={() => { setSubmitted(false); setForm({ type: "", name: "", company: "", phone: "", email: "", message: "" }); setErrors({}); }} style={{ marginTop: 24, background: "transparent", border: "1px solid var(--border)", color: "var(--muted2)", padding: "10px 24px", cursor: "pointer", fontSize: 13, fontFamily: "inherit" }}>Ново запитване</button>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 18 }}>
      {/* Вид запитване */}
      <div className="field">
        <label>Вид запитване</label>
        <select id="contact-first-field" className="finput" value={form.type} onChange={e => setForm({...form, type: e.target.value})} style={{ cursor: "pointer" }}>
          <option value="">— Изберете —</option>
          <option value="Запитване за оферта">Запитване за оферта</option>
          <option value="Запитване за фискализация">Запитване за фискализация</option>
          <option value="Запитване за сервиз">Запитване за сервиз</option>
          <option value="Общо запитване">Общо запитване</option>
        </select>
      </div>

      {/* Незадължителни полета */}
      {[["name","Вашето име","text","Иван Иванов"],["company","Фирма","text","ЕООД / АД"]].map(([n,l,t,p]) => (
        <div key={n} className="field">
          <label>{l}</label>
          <input type={t} placeholder={p} className="finput" value={form[n]} onChange={e => setForm({...form,[n]:e.target.value})} />
        </div>
      ))}

      {/* Телефон — задължителен */}
      <div className="field">
        <label>Телефон *</label>
        <input
          type="tel"
          ...
          style={{ borderColor: errors.phone ? "#E24B4A" : undefined }}
        />
        {errors.phone && <div style={{ fontSize: 12, color: "#E24B4A", marginTop: 4 }}>⚠ {errors.phone}</div>}
      </div>
      ...
      <div className="field">
        ...
        />
        {errors.email && <div style={{ fontSize: 12, color: "#E24B4A", marginTop: 4 }}>⚠ {errors.email}</div>}
      </div>
  ...
  return (
    <div className="page" style={{ padding: "60px 0 80px", background: "var(--bg)", minHeight: "calc(100vh - 60px)" }}>
      <div className="container">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", gap: 32, alignItems: "start" }} className="catalog-layout">

          {/* SIDEBAR */}
          <div style={{ position: "sticky", top: 76, background: "var(--bg2)", border: "1px solid var(--border)" }}>
            {/* Марка */}
            <div style={{ padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--amber)", marginBottom: 14 }}>// Марка</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {brands.map(b => (
                  <button key={b} onClick={() => setActiveBrand(b)} style={{
                    background: activeBrand === b ? "var(--amber)" : "transparent",
                    border: activeBrand === b ? "none" : "none",
                    color: activeBrand === b ? "#000" : "var(--muted2)",
                    fontWeight: activeBrand === b ? 800 : 500,
                    fontSize: 13, padding: "8px 12px", cursor: "pointer",
                    fontFamily: "inherit", textAlign: "left", letterSpacing: "0.04em",
                    transition: "all 0.15s",
                    clipPath: activeBrand === b ? "polygon(5px 0%,100% 0%,calc(100% - 5px) 100%,0% 100%)" : "none",
                  }}
                  onMouseEnter={e => { if(activeBrand !== b) e.target.style.color = "var(--amber)"; }}
                  onMouseLeave={e => { if(activeBrand !== b) e.target.style.color = "var(--muted2)"; }}
                  >{b}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ padding: "16px 20px" }}>
              <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "var(--amber)", marginBottom: 14 }}>// Тип</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {categories.map(c => (
                  <button key={c} onClick={() => setActiveCategory(c)} style={{
                    background: activeCategory === c ? "var(--amber)" : "transparent",
                    border: "none",
                    color: activeCategory === c ? "#000" : "var(--muted2)",
                    fontWeight: activeCategory === c ? 800 : 500,
                    fontSize: 13, padding: "8px 12px", cursor: "pointer",
                    fontFamily: "inherit", textAlign: "left", letterSpacing: "0.04em",
                    transition: "all 0.15s",
                    clipPath: activeCategory === c ? "polygon(5px 0%,100% 0%,calc(100% - 5px) 100%,0% 100%)" : "none",
                  }}
                  onMouseEnter={e => { if(activeCategory !== c) e.target.style.color = "var(--amber)"; }}
                  onMouseLeave={e => { if(activeCategory !== c) e.target.style.color = "var(--muted2)"; }}
                  >{c}</button>
                ))}
              </div>
            </div>
            {/* Reset */}
            {(activeBrand !== "Всички" || activeCategory !== "Всички") && (
              <div style={{ padding: "12px 20px", borderTop: "1px solid var(--border)" }}>
                <button onClick={() => { setActiveBrand("Всички"); setActiveCategory("Всички"); }} style={{
                  background: "none", border: "1px solid var(--border)", color: "var(--muted)",
                  fontSize: 11, padding: "6px 14px", cursor: "pointer", fontFamily: "inherit",
        ...
        <div className="container">
          <div className="svc-two-col">
            <div>
              ...
              <p style={{ fontSize: 16, color: "var(--muted2)", lineHeight: 1.9, marginBottom: 32 }}>
                Работим с устройства от всички одобрени производители: Datecs, Eltrade, Daisy и Tremol. Бърза диагностика, резервни части и документация за НАП.
              </p>
        ...
        <div className="container">
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
            ...
            <p className="hero-sub" style={{ margin: "0 auto 48px" }}>
              Изодимпекс ЕООД — официален дистрибутор и сервиз на водещи производители на фискални устройства. Касови апарати, мобилни решения и фискални принтери за всеки тип бизнес.
            </p>
          ...
          <div className="stat-strip">
            {[["20+","Години опит"],["600+","Обслужени бизнеса"],["4","Производителя"],["24ч","Отговор"]].map(([n,l]) => (
      ...
      <section id="about" className="about-section">
        ...
        <div className="container">
          <div className="about-grid">
            ...
            <div style={{ borderTop: "1px solid var(--border)" }}>
              {["Консултация според типа бизнес и натоварване","Официален дистрибутор на 4 водещи производителя","Фискализация и въвеждане в експлоатация","Сервиз и поддръжка на фискални устройства","Ясна комуникация и коректни условия"].map((item, i) => (
                  ...
                  <div className="faq-body">
                    ...
                    {item.brands && (
                      <div style={{ display: "flex", gap: 16, marginTop: 16, flexWrap: "wrap" }}>
                        {["Datecs","Eltrade","Daisy","Tremol"].map(b => (
                          <span key={b} style={{ background: "var(--amber-glow)", border: "1px solid var(--border-amber)", color: "var(--amber)", fontSize: 13, fontWeight: 800, padding: "6px 18px", letterSpacing: "0.1em", textTransform: "uppercase" }}>{b}</span>
                        ))}
                      </div>
                    )}
                  </div>
...
function PrivacyPage({ goHome }) {
  useEffect(() => { window.scrollTo({ top: 0 }); }, []);
  return (
    <div className="page" style={{ background: "var(--bg)", minHeight: "calc(100vh - 60px)", padding: "60px 0 80px" }}>
      <div className="container" style={{ maxWidth: 800 }}>
        <button className="back-btn" onClick={goHome} style={{ marginBottom: 32 }}>
          <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8l5 5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
          Начало
        </button>
        <div className="section-tag">Правна информация</div>
        <h1 className="section-title" style={{ marginBottom: 40 }}>Политика за поверителност</h1>

        {[
          {
            title: "1. Администратор на лични данни",
            text: `Изодимпекс ЕООД, ЕИК: 121771064, със седалище и адрес на управление: [АДРЕС], МОЛ: [ИМЕ], e-mail: izodimpex@abv.bg, тел: +359 898 444 242.`
          },
          {
            title: "2. Какви данни събираме",
            text: `При попълване на контактната форма събираме: име, фирма, телефонен номер и имейл адрес. Тези данни са необходими единствено за да отговорим на вашето запитване.`
          },
          {
            title: "3. Цел и основание за обработване",
            text: `Данните се обработват с цел отговор на запитване за продукти и услуги (чл. 6, ал. 1, б. "б" от GDPR — изпълнение на преддоговорни отношения). Не използваме данните за маркетингови цели без изрично съгласие.`
          },
          {
            title: "4. Срок на съхранение",
            text: `Личните данни се съхраняват за срок не по-дълъг от 12 месеца от датата на получаване на запитването, след което се изтриват.`
          },
          {
            title: "5. Предаване на данни",
            text: `Не предаваме личните ви данни на трети страни, освен ако това е необходимо по закон или с ваше изрично съгласие.`
          },
          {
            title: "6. Вашите права",
            text: `Имате право на: достъп до данните си, коригиране, изтриване ("право да бъдете забравени"), ограничаване на обработването, преносимост на данните и право на възражение. За упражняване на правата си свържете се с нас на izodimpex@abv.bg.`
          },
          {
            title: "7. Бисквитки (Cookies)",
            text: `Сайтът използва технически необходими бисквитки за нормалното му функциониране. При добавяне на аналитични инструменти (Google Analytics) ще бъдете уведомени и ще ви бъде поискано съгласие.`
          },
          {
            title: "8. Жалби",
            text: `Имате право да подадете жалба до Комисията за защита на личните данни (КЗЛД), адрес: гр. София 1592, бул. „Проф. Цветан Лазаров" № 2, www.cpdp.bg.`
          },
          {
            title: "9. Промени в политиката",
            text: `Запазваме правото си да актуализираме тази политика. Актуалната версия е винаги достъпна на този адрес. Последна актуализация: март 2026 г.`
          },
        ].map(({ title, text }) => (
          <div key={title} style={{ marginBottom: 32, paddingBottom: 32, borderBottom: "1px solid var(--border)" }}>
            <h2 style={{ fontSize: 17, fontWeight: 700, color: "var(--amber)", marginBottom: 12, letterSpacing: "-0.01em" }}>{title}</h2>
            <p style={{ fontSize: 15, color: "var(--muted2)", lineHeight: 1.9 }}>{text}</p>
          </div>
        ))}
      </div>
    </div>
...
// ─── COOKIES BANNER ───────────────────────────────────────────────────────────
function CookiesBanner({ onAccept, onDecline, onCustom }) {
  ...
  return (
    <div style={{
      ...
      background: "var(--bg2)", borderTop: "1px solid var(--border-amber)",
      boxShadow: "0 -4px 24px rgba(0,0,0,0.3)"
    }}>
      {/* Main bar */}
      <div style={{ padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 16 }}>
        <div style={{ fontSize: 14, color: "var(--muted2)", maxWidth: 580, lineHeight: 1.7 }}>
          <span style={{ color: "var(--amber)", fontWeight: 700 }}>Бисквитки</span> — Използваме бисквитки за нормалната работа на сайта и за анализ на посещенията.
          {" "}<span onClick={() => setExpanded(!expanded)} style={{ color: "var(--amber)", cursor: "pointer", borderBottom: "1px solid var(--border-amber)" }}>
            {expanded ? "Скрий детайли ▲" : "Персонализирай ▼"}
          </span>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={onDecline} style={{
            background: "transparent", border: "1px solid var(--border)",
            color: "var(--muted2)", padding: "9px 18px", cursor: "pointer",
            fontSize: 13, fontFamily: "inherit"
          }}>Само необходимите</button>
          <button onClick={onAccept} className="btn-amber" style={{ padding: "9px 24px", fontSize: 13 }}>
            Приемам всички
          </button>
        </div>
      </div>
        ...
        <div style={{ borderTop: "1px solid var(--border)", padding: "20px 28px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 700 }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20 }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>Необходими бисквитки</div>
                <div style={{ fontSize: 13, color: "var(--muted2)", lineHeight: 1.6 }}>Задължителни за работата на сайта. Запомнят вашите предпочитания за бисквитки.</div>
              </div>
              <div style={{ background: "var(--amber)", color: "#000", fontSize: 11, fontWeight: 700, padding: "4px 12px", letterSpacing: "0.08em", flexShrink: 0, marginTop: 2 }}>ВИНАГИ АКТИВНИ</div>
            </div>

            {/* Аналитични */}
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>Аналитични бисквитки</div>
                <div style={{ fontSize: 13, color: "var(--muted2)", lineHeight: 1.6 }}>Google Analytics — помагат ни да разберем как посетителите използват сайта. Данните са анонимни.</div>
              </div>
              {/* Toggle */}
              <div onClick={() => setAnalytics(!analytics)} style={{
                width: 44, height: 24, borderRadius: 12, cursor: "pointer", flexShrink: 0, marginTop: 2,
                background: analytics ? "var(--amber)" : "var(--border)",
                position: "relative", transition: "background 0.2s"
              }}>
                <div style={{
                  position: "absolute", top: 3, left: analytics ? 23 : 3,
                  width: 18, height: 18, borderRadius: "50%",
                  background: analytics ? "#000" : "var(--muted2)",
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20, paddingTop: 16, borderTop: "1px solid var(--border)" }}>
              <div>
                <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 4 }}>Маркетингови бисквитки</div>
                <div style={{ fontSize: 13, color: "var(--muted2)", lineHeight: 1.6 }}>Използват се за персонализирана реклама. В момента не използваме такива бисквитки.</div>
              </div>
              <div style={{ fontSize: 12, color: "var(--muted)", flexShrink: 0, marginTop: 2 }}>НЕ СЕ ИЗПОЛЗВАТ</div>
            </div>

            <div style={{ paddingTop: 16, borderTop: "1px solid var(--border)" }}>
              <button onClick={() => onCustom({ analytics })} className="btn-amber" style={{ fontSize: 13, padding: "10px 24px" }}>
                Запази избора
              </button>
            </div>
          </div>
...
export default function IzodimpexWebsite() {
  const [page, setPage] = useState("home"); // "home" | "products" | "services" | "detail" | "privacy"
  ...
  const goPrivacy = () => { setPage("privacy"); setSelectedProduct(null); window.scrollTo({ top: 0 }); };
  const goProduct = (product) => { setSelectedProduct(product); setPage("detail"); window.scrollTo({ top: 0 }); };
  const goContact = () => {
    const scrollAndFocus = () => {
      document.getElementById("contact")?.scrollIntoView({ behavior: "smooth" });
      setTimeout(() => {
        const firstField = document.getElementById("contact-first-field");
  ...
  return (
    <div style={{ fontFamily: "'Segoe UI', system-ui, sans-serif", background: "var(--bg)", color: "var(--text)", minHeight: "100vh", overflowX: "hidden" }}>
      ...
      {showCookies && <CookiesBanner onAccept={() => { localStorage.setItem("cookies_accepted","true"); localStorage.setItem("cookies_analytics","true"); setShowCookies(false); setCookiesAccepted(true); }} onDecline={() => { localStorage.setItem("cookies_accepted","necessary"); setShowCookies(false); }} onCustom={(prefs) => { localStorage.setItem("cookies_accepted", prefs.analytics ? "true" : "necessary"); localStorage.setItem("cookies_analytics", prefs.analytics ? "true" : "false"); setShowCookies(false); }} />}

      <footer className="footer">
        <div className="container">
          <div style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12, fontSize: 12, color: "var(--muted)", letterSpacing: "0.06em", alignItems: "center" }}>
            <div>© {new Date().getFullYear()} Изодимпекс ЕООД</div>
            <span onClick={goPrivacy} style={{ fontSize: 12, color: "var(--muted)", cursor: "pointer", letterSpacing: "0.06em", borderBottom: "1px solid var(--border)" }}>Политика за поверителност</span>
          </div>
        </div>
      </footer>
